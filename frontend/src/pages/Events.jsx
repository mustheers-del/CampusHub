...
const CATEGORIES = ['All', 'Technical', 'Cultural', 'Sports', 'Workshop', 'Seminar', 'Competition'];
const STATUSES = ['All', 'Upcoming', 'Ongoing', 'Completed'];
const REG_TYPES = ['All', 'Individual', 'Group'];
...
export default function Events() {
  ...
  const loadEvents = async () => {
    try {
      setLoading(true);
      setError(null);
      const res = await getEvents({ search, category, status, registration_type: regType });
      if (res.success) {
        setEvents(res.data);
      } else {
  ...
  const handleSaveEvent = async (formData) => {
    if (editingEvent) {
      ...
      showNotification(`Event "${formData.title}" updated successfully!`);
    } else {
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      showNotification(`Event "${deletingEventTitle}" deleted successfully!`);
  ...
  const handleRegisterStudent = async (eventId, registrationData) => {
    ...
    <div>
      <div className="page-header">
        <div className="page-header-text">
          <h1>{isAdmin ? 'Campus Events Management' : 'Explore Campus Events'}</h1>
          <p>Browse, search, filter, and register for upcoming college activities and group challenges.</p>
        </div>
        {isAdmin && (
          <button 
